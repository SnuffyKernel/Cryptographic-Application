...
std::string md5hash_to_string(std::vector<uint8_t> hash, std::string hex_char, const bool file) {
    try {
        if (hex_char == "") hex_char = "0123456789abcdef";
        if (hex_char.size() > 16) throw std::runtime_error("Exceeded the limit of 16 characters");
        if (hex_char.size() < 16) throw std::runtime_error("Enter " + std::to_string(16 - hex_char.size()) + " more characters");
        std::string ret = "";
        ...
        if (file == 1) {
            std::ofstream file("MD5.txt", std::ios::app);
            if (file.is_open()) {
                file << "Хеш: " << ret << std::endl;
                file.close();
                std::cout << "Writing to the file was successful." << std::endl;
            }
            else throw std::runtime_error("File opening error.");
